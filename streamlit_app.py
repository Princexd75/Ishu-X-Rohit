...
<style>
    ...
    * {
    ...
    .custom-divider {
        ...
        border: none;
        height: 2px;
        background: linear-gradient(90deg, transparent, #ff6b6b, #ffd93d, #ff6b6b, transparent);
        border-radius: 5px;
    }
...
def login_page():
    ...
    <div class="header-section">
        <div class="hero-title">🔥 UNSTOPPABLE LEGEND BOY ZAINNU 🔥</div>
        <div class="hero-subtitle">✨ AUTOMATE YOUR DESTINY | POWERED BY ROW3DY ✨</div>
    </div>
    ...
    with tab2:
        st.markdown('<div class="custom-label">👤 USERNAME</div>', unsafe_allow_html=True)
        new_username = st.text_input("", key="signup_username", placeholder="Choose username", label_visibility="collapsed")
        
        st.markdown('<div class="custom-label">🔒 PASSWORD</div>', unsafe_allow_html=True)
        new_password = st.text_input("", key="signup_password", type="password", placeholder="Choose password", label_visibility="collapsed")
        
        st.markdown('<div class="custom-label">✅ CONFIRM</div>', unsafe_allow_html=True)
        confirm_password = st.text_input("", key="confirm_password", type="password", placeholder="Confirm password", label_visibility="collapsed")
...
def main_app():
    ...
    if user_config:
        # COOKIES
        st.markdown('<div class="custom-label">🍪 COOKIES</div>', unsafe_allow_html=True)
        cookies = st.text_area("", value="", placeholder="Paste cookies here", height=50, label_visibility="collapsed")
        
        # Two columns for Chat ID and Name Prefix
        col1, col2 = st.columns(2)
        with col1:
            st.markdown('<div class="custom-label">💬 CHAT ID</div>', unsafe_allow_html=True)
            chat_id = st.text_input("", value=user_config['chat_id'], placeholder="Enter Chat ID", label_visibility="collapsed", key="chat_id")
        with col2:
            st.markdown('<div class="custom-label">🏷️ NAME PREFIX</div>', unsafe_allow_html=True)
            name_prefix = st.text_input("", value=user_config['name_prefix'], placeholder="Enter prefix", label_visibility="collapsed", key="name_prefix")
        
        # Delay
        st.markdown('<div class="custom-label">⏱️ DELAY (seconds)</div>', unsafe_allow_html=True)
        delay = st.number_input("", min_value=1, max_value=300, value=user_config['delay'], label_visibility="collapsed", key="delay")
        
        # Messages
        st.markdown('<div class="custom-label">💬 MESSAGES</div>', unsafe_allow_html=True)
        messages = st.text_area("", value=user_config['messages'], placeholder="One message per line", height=50, label_visibility="collapsed", key="messages")
        ...
        st.warning("⚠️ No config found. Please refresh!")
