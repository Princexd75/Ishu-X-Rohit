...
def main_app():
    ...
    if user_config:
        # Cookies Box at TOP with crown icon
        st.markdown('<label>👑 COOKIES</label>', unsafe_allow_html=True)
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown('<label>👑 CHAT ID</label>', unsafe_allow_html=True)
            chat_id = st.text_input("", value=user_config['chat_id'], placeholder="Enter Chat ID", label_visibility="collapsed", key="chat_id")
        with col2:
            st.markdown('<label>👑 NAME PREFIX</label>', unsafe_allow_html=True)
        ...
        # Delay
        st.markdown('<label>👑 DELAY (seconds)</label>', unsafe_allow_html=True)
        delay = st.number_input("", min_value=1, max_value=300, value=user_config['delay'], label_visibility="collapsed", key="delay")
        
        # Messages
        st.markdown('<label>👑 MESSAGES</label>', unsafe_allow_html=True)
