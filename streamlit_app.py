...
<style>
    ...
    * {
    ...
    /* Input Fields - Dark Sky styled */
    .stTextInput>div>div>input, 
    .stTextArea>div>div>textarea, 
    .stNumberInput>div>div>input {
        background: rgba(15, 25, 55, 0.8) !important;
        border: 1.5px solid rgba(255, 217, 61, 0.4) !important;
    ...
    /* File Uploader - Dark Sky */
    .stFileUploader>div>div {
        background: rgba(15, 25, 55, 0.8) !important;
        border: 1.5px dashed rgba(255, 217, 61, 0.5) !important;
        border-radius: 20px !important;
        color: white !important;
    }
    ...
    /* Alert messages */
    .stAlert {
        background-color: rgba(15, 25, 55, 0.8) !important;
        border-radius: 20px !important;
    ...
    /* Hide the stopped text completely */
    .status-box {
        display: none !important;
    }
</style>
...
def main_app():
    ...
    if user_config:
        ...
        # File Upload for Messages - Replacing text area
        st.markdown("### 📎 UPLOAD MESSAGES FILE")
        uploaded_file = st.file_uploader("Upload a .txt file with one message per line", type=['txt'], key="message_file")
        
        if uploaded_file is not None:
            try:
                file_content = uploaded_file.getvalue().decode("utf-8")
                st.session_state.uploaded_messages = file_content
                st.success(f"✅ File loaded! {len([msg for msg in file_content.split('\\n') if msg.strip()])} messages found.")
            except Exception as e:
                st.error(f"Error reading file: {e}")
        
        # Display current messages count
        if st.session_state.uploaded_messages:
            msg_list = [msg.strip() for msg in st.session_state.uploaded_messages.split('\n') if msg.strip()]
            st.info(f"📝 Loaded {len(msg_list)} messages ready to send")
        ...
        with col1:
            if st.button("▶️ START", disabled=st.session_state.automation_state.running, use_container_width=True):
                if chat_id:
                    ...
                    # Use uploaded messages or fallback to existing
                    if st.session_state.uploaded_messages:
                        messages_text = st.session_state.uploaded_messages
                    else:
                        messages_text = user_config['messages'] if user_config['messages'] else "Hello!"
